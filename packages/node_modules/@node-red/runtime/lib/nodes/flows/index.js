...
function setFlows(_config,type,muteLog,forceStart, user) {
    ...
    try {
        if(user){
            _config = _config.map(function(n){n.owner = redUtil.getOwner(user); return n});
        }
    } catch (error) {
        console.log(error);
    }
    ...
    if (type === "load") {
        ...
        // GAIA Multitenant
        var oldConfig = clone(activeConfig.flows);
        if(config.length>0){
            oldConfig.forEach(function(item){
            if(!redUtil.isOwner(item.owner, user)){
                config.push(item);
            }
            });
         }
            ...
            if (forceStart || started) {
                ...
                return stop(type,diff,muteLog).then(() => {
                    ...
                    start(type,diff,muteLog).then(() => {
                        // GAIA Multitenant
                        events.emit("runtime-event",{id:"runtime-deploy",payload:{revision:flowRevision, owner:user},retain: true});
                    });
...
function getFlows(user) {
    // GAIA Multitenant
    tmpConfig = clone(activeConfig);
    tmpConfig.flows = tmpConfig.flows.filter(function(n){return redUtil.isOwner(n.owner, user)});
    return tmpConfig;
}
